...
if (isset($_POST['inv_type'])) {
	?>
        ...
        $gr = $_POST['grade_lvl'];
        if($gr != "all"){
            $sql="SELECT * FROM learning_materials As lm INNER JOIN tbl_request AS tr ON lm.lm_id=tr.stat_id WHERE tr.stat = 'Done' AND lm.materialtype='$type' AND lm.grade_lvl = '$gr'";
        }else{
            $sql="SELECT * FROM learning_materials As lm INNER JOIN tbl_request AS tr ON lm.lm_id=tr.stat_id WHERE tr.stat = 'Done' AND lm.materialtype='$type'";
        }
        ...
        ?>
            <tr>
                <td class="bg-light" style="text-align: center;">
                    <i class="bi-pencil-square text-primary" style="cursor: pointer;" onclick="editM(<?=$row['lm_id']?>)"></i>
                </td>
                ...
                <td style = "text-align:center;"><?=$row['lm_remark']?></td>
            </tr>
